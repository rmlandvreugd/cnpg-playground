...
|---|---|---|
| Edge Traefik (host container) | `172.28.0.250` → `*.172-28-0-250.sslip.io` | vault, authelia, seaweedfs S3, seaweedfs-admin |
| In-cluster Traefik (metallb LB) | `172.28.255.200` → `*.172-28-255-200.sslip.io` | grafana, argocd, gangplank, radar, in-cluster authelia portal |
| Authelia edge portal | `authelia.172-28-0-250.sslip.io` | host containers |
| Authelia in-cluster portal | `authelia.172-28-255-200.sslip.io` | in-cluster clients (hub → edge → Authelia) |
...
Both `172.28.0.250` and `172.28.255.x` are on the docker/kind network and are
...
- **Bash/mc**: `mc alias set edge https://seaweedfs.172-28-0-250.sslip.io ...`
...
For `seaweedfs-admin.172-28-0-250.sslip.io` (edge, forward-auth middleware):
1. `browser_navigate` to the admin URL **unauthenticated**.
2. `browser_network_requests` → assert a **302** whose redirect lands on
   `authelia.172-28-0-250.sslip.io` (the edge portal), and that the final page is
...
- Repeat the unauth→302 probe against `vault.172-28-0-250.sslip.io` to confirm
...
- **Playwright**: `browser_navigate` to an in-cluster human surface behind the
  in-cluster portal — e.g. `argocd.172-28-255-200.sslip.io` (or grafana).
- Assert redirect to `authelia.172-28-255-200.sslip.io` (the **in-cluster**
...
- **k8s**: confirm the per-request OIDC issuer — `pods_exec` a curl to
  `https://authelia.172-28-255-200.sslip.io/.well-known/openid-configuration`
...
- Cookie scope: the edge portal issues cookies for `172-28-0-250.sslip.io`, the
  in-cluster portal for `172-28-255-200.sslip.io` — a login on one does **not**
...
`172.28.0.250`, in-cluster Traefik `172.28.255.200`, tenant DB `172.28.255.210`.
...
both the edge portal (`authelia.172-28-0-250.sslip.io`) and the in-cluster portal
(`authelia.172-28-255-200.sslip.io`, which proxies to the edge via the
...
- **Root cause:** the `authelia` host container is on the docker **`bridge`**
  ...
  (`172.28.0.250`) → cannot reach `authelia:9091`. vault (`172.28.0.13`) and
  seaweedfs-admin (`172.28.0.15`) are on **both** and route fine. Authelia itself
...
| 1 | Edge is single front | ✅ PASS | `traefik-edge` Up 10h, `:443`, IP `172.28.0.250` on kind net; vault routes through edge → 200. |
...
| 7 | Tenant URLs (added) | ⚠️ MIXED | **pgAdmin** `http://pgadmin-rbr-ver.172-28-255-200.sslip.io` → 301→**200** (own login, Authelia-independent) ✅. **tenant Grafana** `https://grafana-rbr-ver.172-28-255-200.sslip.io` → 302 `/login` (page loads) but "Sign in with Authelia" hits the dead portal ⚠️. |
...
| 4 | in-cluster portal OIDC issuer | 504 | **`https://authelia.172-28-255-200.sslip.io`** (in-cluster host) ✅ |
...
- Unauthenticated `https://seaweedfs-admin.172-28-0-250.sslip.io/`
  → 302 to edge portal `authelia.172-28-0-250.sslip.io/?rd=...seaweedfs-admin...`.
...
- `https://grafana-rbr-ver.172-28-255-200.sslip.io/` shows **"Sign in with Authelia"**.
- Click → redirects to the **in-cluster portal** `authelia.172-28-255-200.sslip.io/?flow=openid_connect`
...
- in-cluster OIDC issuer (from pgadmin pod) → **`https://authelia.172-28-255-200.sslip.io`**
