...
| `VAULT_K8S_ADDR` | `https://vault.172-28-0-250.sslip.io` | Vault address reachable by cert-manager (canonical edge-LB URL; no port) |
...
```
https://vault.172-28-0-250.sslip.io      # <edge-ip-dashed>.sslip.io, port 443, no :8200
```

The edge terminates TLS at `:443`, then does a **verified re-encrypt** to
`https://vault:8200` (`serverName: vault.172-28-0-250.sslip.io`, a SAN on Vault's
...
presents its own step-ca-issued cert for `vault.172-28-0-250.sslip.io`.
