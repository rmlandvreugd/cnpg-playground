...
AUTHELIA_RADAR_CLIENT_SECRET_HASH=$(_hash_secret "${AUTHELIA_RADAR_CLIENT_SECRET}")
...
AUTHELIA_RADAR_CLIENT_SECRET_HASH="${AUTHELIA_RADAR_CLIENT_SECRET_HASH}" \
envsubst '${AUTHELIA_HOST} ${AUTHELIA_PORT} ${HOST_IP_DASHED} ${AUTHELIA_JWT_SECRET} ${AUTHELIA_SESSION_SECRET} ${AUTHELIA_STORAGE_ENCRYPTION_KEY} ${AUTHELIA_OIDC_HMAC_SECRET} ${VAULT_HOST} ${VAULT_PORT} ${STEP_CA_HOST} ${STEP_CA_PORT} ${TRAEFIK_IP_DASHED} ${AUTHELIA_VAULT_CLIENT_SECRET_HASH} ${AUTHELIA_STEP_CA_CLIENT_SECRET_HASH} ${AUTHELIA_GRAFANA_RBR_VER_CLIENT_SECRET_HASH} ${AUTHELIA_GRAFANA_MONITORING_CLIENT_SECRET_HASH} ${AUTHELIA_RADAR_CLIENT_SECRET_HASH}' \
    < "${AUTHELIA_CONFIG_DIR}/configuration.yaml.tpl" \
