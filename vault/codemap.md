...
- **PKI hierarchy**: 3-tier hierarchy — step-ca Root CA → step-ca Intermediate CA → Vault Intermediate CA → leaf certificates for **in-cluster** workloads only (PostgreSQL instances, Traefik, Barman Cloud Plugin, mTLS clients). External services (Vault itself, Dex) receive their TLS certificates directly from step-ca's intermediate CA via the X5C provisioner, not from Vault PKI.
...
- **2 issuance roles** (`cluster-certs`, `mtls-client`) with distinct `allowed_domains` constraints enforce which in-cluster workloads can request certificates for which DNS names. The `mtls-client` role defines `client_flag=true server_flag=false` and a shorter max TTL (168h). Both roles set `not_before_duration=0s` to prevent "notBefore before signer's notBefore" errors. The former `dex-server` role has been removed — Dex now receives its TLS cert directly from step-ca's intermediate CA via the X5C provisioner.
...
- **TLS-only primary API**: Vault API is served over TLS (port 8200) using a certificate issued by step-ca's intermediate CA via the X5C provisioner (stored in `vault/certs/vault-cert.pem`, `vault-key.pem`). A plain-text listener on port 8202 exists only as a bootstrap fallback. In-cluster consumers verify TLS using the step-ca root chain (`vault-ca.pem`).
- **Vault's own TLS cert issued by step-ca via X5C**: During `vault-setup.sh`, the script requests a server certificate from step-ca using the X5C provisioner (with the step-ca intermediate CA cert+key) with SANs for the sslip.io hostname (`vault.<host-ip-dashed>.sslip.io`), localhost, and the bridge-network IP. The full chain (step-ca root + intermediate) is concatenated into `vault-ca.pem` for client verification.
...
| **Dex** (OIDC) | step-ca X5C provisioner | TLS cert issued by step-ca intermediate | step-ca intermediate + root CA chain |
...
| Path | Purpose |
|------|---------|
| `config/` | Vault server HCL configuration (listener, storage, telemetry) |
| `cert-manager/` | cert-manager ClusterIssuer templates for Vault PKI |
| `trust-manager/` | trust-manager Bundle combining step-ca roots + Vault PKI intermediate for mTLS verification |
| `eso/` | External Secrets Operator ClusterSecretStore templates |
| `traefik/` | K8s Service/Endpoints wiring Vault into the cluster |
